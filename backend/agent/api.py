...
async def run_agent_background(
    ...
    """Run the agent in the background with enhanced error handling and logging."""
    client = await db.client
    logger.info(f"Starting background agent run {agent_run_id} for thread {thread_id} with model {model_name}")
    ...
    try:
        # Set up Redis keys and channels
        await redis.set(active_run_key, "1")
        logger.debug(f"Set active run key: {active_run_key}")
        
        # Subscribe to control channels
        pubsub = redis.pubsub()
        await pubsub.subscribe(global_control_channel, instance_control_channel)
        logger.debug(f"Subscribed to control channels for {agent_run_id}")
        
        async def check_for_stop_signal():
            """Check for stop signals from Redis channels."""
            try:
                while True:
                ...
                logger.error(f"Error in stop signal checker for {agent_run_id}: {str(e)}")
                return True
        ...
        try:
            # Run the agent
            logger.info(f"Starting agent execution for run {agent_run_id}")
            await run_agent(
            ...
            # Clean up resources
            logger.info(f"Cleaning up resources for agent run {agent_run_id}")
            try:
                # Cancel the stop checker
                stop_checker.cancel()
                try:
                    await stop_checker
                except asyncio.CancelledError:
                ...
                await pubsub.unsubscribe(global_control_channel, instance_control_channel)
                await pubsub.close()
                ...
                logger.info(f"Successfully cleaned up resources for agent run {agent_run_id}")
                
            except Exception as cleanup_error:
                logger.error(f"Error during cleanup for agent run {agent_run_id}: {str(cleanup_error)}")
                
    except Exception as e:
        error_msg = f"Critical error in agent run {agent_run_id}: {str(e)}"
        logger.error(error_msg, exc_info=True)
        await update_agent_run_status(client, agent_run_id, "failed", error=error_msg)
        ...
        try:
            ...
            logger.error(f"Emergency cleanup failed for agent run {agent_run_id}: {str(cleanup_error)}")
