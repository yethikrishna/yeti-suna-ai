...
class FirecrawlTool(SandboxToolsBase):
    """Tool for performing web scraping using Firecrawl."""
    ...
    @openapi_schema({
        ...
        "function": {
            "name": "firecrawl",
            "description": "Extract full text content from multiple webpages using Firecrawl. This tool provides efficient scraping with markdown conversion and handling of dynamic content. IMPORTANT: Always collect multiple relevant URLs and scrape them all in a single call for efficiency.",
            "parameters": {
                "type": "object",
                "properties": {
                    "urls": {
    ...
    @xml_schema(
        ...
        example='''
        <!-- 
        The firecrawl tool uses Firecrawl to efficiently extract content from multiple webpages.
        
        WORKFLOW PRIORITY:
        1. Use web-search first to find relevant URLs
        2. Then use firecrawl to extract content from multiple URLs at once
        ...
        - Manages proxies and rate limits
    ...
    async def firecrawl(
        ...
        """
        Extract content from multiple webpages using Firecrawl.
        
        Parameters:
        - urls: Multiple URLs to scrape, separated by commas
        """
        try:
            logging.info(f"Starting Firecrawl scraping for URLs: {urls}")
            ...
            logging.info(f"Processing {len(url_list)} URLs with Firecrawl")
            ...
            for url in url_list:
                try:
                    ...
                    # Scrape this URL using Firecrawl
            ...
            if successful == len(results):
                message = f"Successfully scraped all {len(results)} URLs using Firecrawl. Results saved to:"
                for r in results:
                    if r.get("file_path"):
                        message += f"\n- {r.get('file_path')}"
            elif successful > 0:
                message = f"Scraped {successful} URLs successfully and {failed} failed using Firecrawl. Results saved to:"
                for r in results:
                ...
                return self.fail_response(f"Failed to scrape all {len(results)} URLs using Firecrawl. Errors: {error_details}")
            ...
            error_message = str(e)
            logging.error(f"Error in firecrawl: {error_message}")
    ...
    async def _scrape_single_url(self, url: str) -> dict:
        """
        Helper function to scrape a single URL using Firecrawl and return the result information.
        """
        logging.info(f"Scraping single URL with Firecrawl: {url}")
        ...
        try:
            ...
            safe_filename = f"{timestamp}_{domain}_firecrawl.json"
            ...
            error_message = str(e)
            logging.error(f"Error scraping URL '{url}' with Firecrawl: {error_message}")
...
if __name__ == "__main__":
    async def test_firecrawl():
        """Test function for the Firecrawl tool"""
