...
def print_banner():
    """Print OMNI Operator setup banner"""
    print(f"""
...
def check_omni_operator_directory():
    """Check if we're in an OMNI Operator repository"""
    required_dirs = ['backend', 'frontend']
    required_files = ['README.md', 'docker-compose.yaml']
    
    for directory in required_dirs:
        if not os.path.isdir(directory):
            print_error(f"'{directory}' directory not found. Make sure you're in the OMNI Operator repository root.")
            return False
    
    for file in required_files:
        if not os.path.isfile(file):
            print_error(f"'{file}' not found. Make sure you're in the OMNI Operator repository root.")
            return False
    
    print_success("OMNI Operator repository detected")
    return True
...
def collect_daytona_info():
    ...
    print_info(f"Enter 'omni/operator:0.1.2.8' as the image name")
...
def collect_llm_api_keys():
    ...
    print_info("You need at least one LLM provider API key to use OMNI Operator")
...
def configure_backend_env(env_vars, use_docker=True):
    ...
    env_content = """# Generated by OMNI Operator setup script
...
def start_omni_operator():
    """Start OMNI Operator using Docker Compose or manual startup"""
    print_info("You can start OMNI Operator using either Docker Compose or by manually starting the frontend, backend and worker.")

    print(f"\n{Colors.CYAN}How would you like to start OMNI Operator?{Colors.ENDC}")
    ...
    if use_docker:
        print_info("Starting OMNI Operator with Docker Compose...")
        ...
        try:
            ...
            if "backend" in result.stdout and "frontend" in result.stdout:
                print_success("OMNI Operator services are up and running!")
            else:
                print_warning("Some services might not be running correctly. Check 'docker compose ps' for details.")
            
        except subprocess.SubprocessError as e:
            print_error(f"Failed to start OMNI Operator: {e}")
            sys.exit(1)
...
def final_instructions(use_docker=True, env_vars=None):
    ...
    print(f"\n{Colors.GREEN}{Colors.BOLD}✨ OMNI Operator Setup Complete! ✨{Colors.ENDC}\n")
    ...
    if env_vars and 'llm' in env_vars and 'MODEL_TO_USE' in env_vars['llm']:
        ...
        print_info(f"OMNI Operator is configured to use {Colors.GREEN}{default_model}{Colors.ENDC} as the default LLM model")
    
    if use_docker:
        print_info("Your OMNI Operator instance is now running!")
        print_info("Access it at: http://localhost:3000")
        print_info("Create an account using Supabase authentication to start using OMNI Operator")
        print("\nUseful Docker commands:")
        print(f"{Colors.CYAN}  docker compose ps{Colors.ENDC}         - Check the status of OMNI Operator services")
        ...
        print(f"{Colors.CYAN}  docker compose down{Colors.ENDC}       - Stop OMNI Operator services")
        print(f"{Colors.CYAN}  docker compose up -d{Colors.ENDC}      - Start OMNI Operator services (after they've been stopped)")
    else:
        print_info("OMNI Operator setup is complete but services are not running yet.")
        print_info("To start OMNI Operator, you need to:")
        ...
        print_info("4. Once all services are running, access OMNI Operator at: http://localhost:3000")
        print_info("5. Create an account using Supabase authentication to start using OMNI Operator")
...
def main():
    ...
    # Print banner
    print_banner()
    print("This wizard will guide you through setting up OMNI Operator, an open-source generalist AI agent.\n")
    ...
    # Check if we're in the OMNI Operator repository
    if not check_omni_operator_directory():
        print_error("This setup script must be run from the OMNI Operator repository root directory.")
        print_info("Please clone the repository first with:")
        print_info("  git clone https://github.com/omni-ai/operator.git")
        print_info("  cd operator")
    ...
    # Install dependencies before starting OMNI Operator
    ...
    # Now ask how to start OMNI Operator
    print_step(current_step, total_steps, "Starting OMNI Operator")
    use_docker = start_omni_operator()
